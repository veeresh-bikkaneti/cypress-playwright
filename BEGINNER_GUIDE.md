...
> **New to TypeScript?** Check out our [Progressive TypeScript Guide](TYPESCRIPT_FOR_CYPRESS.md) first!
> **Using Docker?** See our [Docker Helper Guide](DOCKER_HELPER.md).
...
### 1. "Cypress command not found"
If you try to run `cypress open` and it fails:
- **Solution**: Always use `npx cypress open` or the npm scripts: `npm run cy:open`.

### 2. "Port 3000 already in use"
The test app tries to start on port 3000.
- **Solution**: Find the process using that port and kill it, or restart your computer.

### 3. "Types are missing" (Red underlines in VS Code)
- **Solution**:
  1. Open a TypeScript file.
  2. Press `Ctrl+Shift+P` (or Cmd+Shift+P).
  3. Type "Restart TS Server" and select it.
