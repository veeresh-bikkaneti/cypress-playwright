...
class LoginPage {
  ...
  get rememberCheckbox() {
    return cy.get('[data-testid="remember-checkbox"]');
  }
  get loginAlert() {
    return cy.get('[data-testid="login-alert"]');
  }
  get emailError() {
  ...
  get backToHomeLink() {
    return cy.get('[data-testid="back-to-home-link"]');
  }

  // Navigation link on home page (for tests that start from home)
  get navLoginLink() {
    return cy.get('[data-testid="nav-login"]');
  }
