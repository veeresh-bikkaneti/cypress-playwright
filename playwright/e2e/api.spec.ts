...
test.describe("API Testing - Network Capabilities", () => {
  ...
  test.describe("page.route() - Response Stubbing", () => {
    ...
    test("should match URL patterns for products endpoint", async ({
      page,
    }) => {
    ...
    test("should display product details correctly - mocked", async ({
      page,
    }) => {
      await page.route("**/api/products", async (route) => {
        const json = {
          products: [{ id: 1, name: "Mock Product", price: 99.0 }],
        };
        await route.fulfill({ json });
      });
