...
if (fs.existsSync(TERMINAL_LOG)) {
  ...
  const criticalErrors = logContent
    .split("\n")
    .filter((l) => l.includes("Error:") || l.includes("Failed"));
  console.log(
    ` Found ${criticalErrors.length} critical errors in terminal log.`,
  );
...
console.log(
  " Diagnosis Complete. Please share these findings with the Cypress Healer agent.",
);
