...
async function executeGraphQL(query, variables, req) {
    ...
    // Extract operation name (simplified parsing)
    let data = {};

    // Handle products list query (not single product query)
    // More specific matching: look for 'products' but NOT 'product(' which indicates single product
    const isProductsListQuery = (query.includes('products') || query.includes('GetProducts')) &&
    ...
    if (query.includes('product(') || query.includes('product (')) {
        const id = variables.id || parseInt(query.match(/id:\s*(\d+)/)?.[1] || 1);
    ...
    if (query.includes('user') || query.includes('me')) {
        const token = req.headers.authorization?.replace('Bearer ', '') || req.cookies.authToken;
    ...
    if (query.includes('orders') && !query.includes('createOrder')) {
        const token = req.headers.authorization?.replace('Bearer ', '') || req.cookies.authToken;
    ...
    if (isMutation && query.includes('createOrder')) {
        const token = req.headers.authorization?.replace('Bearer ', '') || req.cookies.authToken;
        ...
        if (items.length === 0) {
            return {
                data: null,
                errors: [{ message: 'Order items are required' }]
            };
        }
        ...
        for (const item of items) {
            ...
            if (!product) {
                return {
                    data: null,
                    errors: [{ message: `Product ${item.productId} not found` }]
                };
            }
    ...
    if (isMutation && query.includes('updateProduct')) {
        const token = req.headers.authorization?.replace('Bearer ', '') || req.cookies.authToken;
        ...
        if (productIndex === -1) {
            return {
                data: null,
                errors: [{ message: `Product ${id} not found` }]
            };
        }
