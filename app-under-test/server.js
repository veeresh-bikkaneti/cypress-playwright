...
server2.on("error", (err) => {
  if (err.code === "EADDRINUSE") {
    console.warn(
      `Warning: Secondary port ${PORT_SECONDARY} already in use – skipping.`,
    );
  }
});
...
app.get("/api/error/:code", (req, res) => {
  ...
  res
    .status(code)
    .json({ error: errorMessages[code] || "Unknown Error", code });
});
...
async function executeGraphQL(query, variables, req) {
  ...
  const isProductsListQuery =
    (compactQuery.includes("products{") ||
      compactQuery.includes("products(")) &&
    !compactQuery.includes("product(");
      ...
      if (!user || user.role !== "admin") {
        ...
        if (productIndex === -1) {
          ...
          if (input.inStock !== undefined)
            products[productIndex].inStock = input.inStock;
