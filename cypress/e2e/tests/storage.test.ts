...
describe('Storage Testing - Cookies & Local Storage', () => {
    ...
    beforeEach(() => {
        ...
        // Setup mock authentication for dashboard access
        cy.window().then((win) => {
            win.localStorage.setItem('authToken', 'mock-token');
            win.localStorage.setItem('user', JSON.stringify({ id: 1, name: 'Test User', email: 'test@example.com' }));
        });
    });
    ...
    describe('Authentication State Management', () => {
        ...
        it('should demonstrate storage isolation', () => {
            // Clear storage manually as beforeEach sets it up
            cy.clearCookies();
            cy.clearLocalStorage();
            cy.clearAllSessionStorage();
